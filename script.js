...
function onChange(r, g, b) {
	...
	this.source.style.backgroundColor = this.color(r, g, b);
...
function onInputChange() {
	...
	this.source.style.backgroundColor = CP.HEX(this.get());
	this.source.style.color = invertColor(CP.HEX(this.get()));
	updateMass();
}

for (i = 0; i < colorNames.length; i++) {
	colors[i] = new CP(document.getElementById(colorNames[i]));
}
